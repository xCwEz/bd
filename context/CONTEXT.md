# CONTEXT.md

> Mon contexte personnel et professionnel pour mon Jarvis.
> Ce fichier sera rempli automatiquement lors de l'installation initiale puis mis à jour au fil du temps par Claude.

---

## Qui je suis

- **Prénom :** Mickael Leyssenne
- **Ville / Pays :** Paris, France
- **Situation actuelle :** Indépendant, travaille seul, en phase de construction de son activité (pas encore de premier client)
- **Profil dominant :** Indépendant / Freelance

---

## Ce que je fais

### Activité principale

Directeur Artistique, Designer UI/UX et Concepteur d'expériences numériques immersives. Travail au carrefour du design graphique, de l'ergonomie web, du développement front de pointe et de la 3D. Ne crée pas des sites statiques, mais des univers interactifs portés par le storytelling et le mouvement (éléments 3D, animations cinématiques).

### Détails selon le profil

**Entrepreneur / Indépendant :**
- Activité : DA / UI-UX / expériences web immersives (3D, animations cinématiques)
- Modèle économique : pas encore défini, à structurer (missions freelance à priori)
- Clients types : aucun client à ce jour, premier client pas encore décroché

---

## Mes objectifs

### Objectifs court terme (3 à 6 mois)

- Finaliser et publier son portfolio / site vitrine immersif
- Décrocher ses 3 premiers clients
- Structurer son offre et ses tarifs freelance

### Objectifs long terme (1 à 3 ans)

- Avoir construit une activité freelance stable en DA/UI/UX/3D avec un portefeuille clients récurrent

---

## Mes projets en cours

Liste des projets ou chantiers actifs sur lesquels je veux que Claude m'aide :

- Site portfolio personnel (création, structuration, publication) : expérience 3D immersive sur le thème d'un univers spatial (planètes, météorites), navigable en scroll/clic. Référence de niveau technique et d'animation : igloo.inc (Three.js, post-processing avancé, GSAP CustomEase, particules, matériaux physiques). L'option précédente (showroom voiture 3D) est abandonnée au profit de ce thème.
- Site vitrine pour **Bourganeuf Automobiles** (garage automobile, Bourganeuf, Creuse) : premier client freelance décroché. Deadline : 2 semaines (à partir du 10/08/2026). Porte de garage 3D procédurale, scroll storytelling, page Contact, section Collection (catalogue de véhicules à vendre, photos réelles du client, reste du site 100% procédural). Base technique : projet Next.js + React Three Fiber + GSAP ScrollTrigger déjà entamé, repris et poursuivi. Contenu réel du client (photos, stock, coordonnées) pas encore reçu, le site avance avec des placeholders clairement identifiés.
- **jarvis-main** (projet perso, `context/import/jarvis-main/`) : script Python d'automatisation de bureau, déclenché par un double-clap au micro (lance Spotify, ouvre Claude/Binance dans Chrome, focus Cursor, voix de bienvenue ElevenLabs). Stade prototype : Windows uniquement, config manuelle via `.env`, mono-poste, pas de packaging ni d'interface. Projet parallèle, sans lien avec l'activité DA/UI-UX.
- **Youtube-automation-main** (projet perso, `context/import/Youtube-automation-main/`) : app Next.js locale qui automatise la création de YouTube Shorts (script → voix ElevenLabs → scènes générées par Kling AI → montage ffmpeg → publication YouTube). Niche : "History's almosts" (quasi-catastrophes historiques), format 9:16, cadence visée 3 Shorts/semaine. État actuel : 1 seul épisode produit sur 10 topics prévus (publié en unlisted sur YouTube, toujours en brouillon sur TikTok), style visuel pas encore verrouillé. Projet parallèle, sans lien avec l'activité DA/UI-UX.

---

## Mes outils et préférences

### Outils que j'utilise au quotidien

- **Design & UI :** Figma
- **3D :** Spline
- **Dev front & motion :** Three.js / React Three Fiber, GSAP (dont ScrollTrigger), VS Code / Cursor

### Style de communication préféré

Direct et efficace

### Domaine où j'ai besoin du plus d'aide

Structuration de l'offre freelance et création du site portfolio, de A à Z (création jusqu'à la publication)

---

## Notes importantes

> Cette section se remplira au fil du temps avec les éléments de contexte qui émergent naturellement dans mes sessions avec Claude.

### Techno du portfolio : Spline vs niveau igloo.inc

Le portfolio vise le niveau technique et de finition d'igloo.inc, qui repose sur du Three.js custom pointu (shaders, post-processing, chargement de modèles optimisés, matériaux physiques). Spline est très bon pour prototyper vite et tester des idées, mais il plafonne pour ce niveau de finition. À garder en tête : Spline pour le prototypage et l'itération rapide, mais la version finale immersive devra être construite en Three.js / React Three Fiber pour atteindre la qualité visée.
